...
<projects>
  ...
  The webapp API helper (src/lib/api.ts) should use:
    import.meta.env.VITE_BACKEND_URL || ""
  
  Empty string fallback makes requests relative (/api/...), which works on any domain behind the reverse proxy.
</projects>
...
<coordination>
  ...
  3. Test backend with cURL (use $BACKEND_URL if available, or the deployed backend URL)
